...
import { useCredentialSignIn, useGoogleSignIn } from "@/tenstack-hooks/useSignin";
import { signIn } from "@/lib/auth";
import { googleSignin } from "@/app/action";
...
export default function SignIn() {
  ...
  const credentialMutation = useCredentialSignIn();
  const googleMutation = useGoogleSignIn();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    toast.promise(
      credentialMutation.mutateAsync({ email, password }),
      {
        loading: 'Signing in...',
        success: (data) => {
          router.push("/dashboard");
  ...
  const handleGoogleSignIn = () => {
    toast.promise(
      ...
      {
        loading: 'Initiating Google sign-in...',
        success: (data) => {
          if (data.url) {
            window.location.href = data.url;
          }
          return 'Google sign-in initiated!';
        },
        error: (err) => `Error: ${err.message}`,
      }
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-cyan-500 to-blue-500 py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}  action={async () => {
        googleSignin();
      }}>
          <input type="hidden" name="remember" defaultValue="true" />
          <div className="rounded-md shadow-sm -space-y-px">
